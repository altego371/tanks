...
func (w *World) escapeAction(me Tank, medTarget *Pt) string {
	...
	if !inFireLane && !inRad && !bulletSoon {
		return ""
	}
...
func decide(r Req) string {
	...
	// ════════════════════════════════════════════════════════
	// ALWAYS: Shoot if we can — clear LOS or through walls
	// Shooting costs nothing if enemy is there
	// ════════════════════════════════════════════════════════
	...
	// If we're in danger, get out NOW
	// ════════════════════════════════════════════════════════
	...
	// ════════════════════════════════════════════════════════
	// ROTATE GUN TOWARD ENEMY — try to get a shot lined up
	// Prefer enemy on axis (even through walls = speculative fire)
	// ════════════════════════════════════════════════════════
	{
		bestDir := Direction("")
		bestS := math.MaxInt32
		...
		if bestDir != "" {
			// only rotate if our current cell is safe
	...
	// ════════════════════════════════════════════════════════
	// PRE-POSITION: if radiation will reach us soon, move early
	// ════════════════════════════════════════════════════════
	if w.futureIrrad(me.X, me.Y, 25) {
		if a := w.moveTo(me, Pt{w.CX, w.CY}); a != "" {
			return a
		}
	}
